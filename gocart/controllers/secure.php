...
class Secure extends Front_Controller {
	...
	function login($ajax = false)
	{
		...
		if ($submitted)
		{
			...
			//customised to red merchant that they are using phone number to register
			$phone		= $this->input->post('phone');			
			$password	= $this->input->post('password');
			$remember   = $this->input->post('remember');
			$redirect	= $this->input->post('redirect');
			$login		= $this->Customer_model->phone_login($phone, $password, $remember);
			if ($login)
			{
				if ($redirect == '')
				{
	...
	function check_phone($str)
	{
		if(!empty($this->customer['id']))
		{
			$phone = $this->Customer_model->check_phone($str, $this->customer['id']);
		}
		else
		{
			$phone = $this->Customer_model->check_phone($str);
		}
	
		if ($phone)
		{
			$this->form_validation->set_message('check_phone', lang('error_phone'));
			return FALSE;
		}
		else
		{
	...
	function my_account($offset=0)
	{
		...
		// solve first time direct go in error
		$this->customer = $this->go_cart->customer();
		...
		$data['page_title']			= 'Welcome '.$data['customer']['name'];
